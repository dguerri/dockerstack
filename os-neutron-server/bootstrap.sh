...
sed -i -e "s#%DATABASE_CONNECTION%#${DATABASE_CONNECTION}#" \
    -e "s#%NEUTRON_NOVA_URL%#${NEUTRON_NOVA_URL}#" \
    -e "s#%NEUTRON_IDENTITY_URI%#${NEUTRON_IDENTITY_URI}#" \
    ...
    -e "s#%NOVA_AUTH_URL%#${NOVA_AUTH_URL}#" \
...
sed -i -e "s#%NEUTRON_EXTERNAL_NETWORKS%#${NEUTRON_EXTERNAL_NETWORKS}#" \
    -e "s#%TUNNEL_LOCAL_IP%#${TUNNEL_LOCAL_IP}#" "$PLUGIN_ML2_CONFIG_FILE" \
